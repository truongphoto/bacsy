...
(() => {
  const APP_VERSION='v1.0.25',BUILD_ID='25';
  ...
  const logoImg=new Image(); logoImg.decoding='async'; logoImg.src='logo-gpp.png?v=25';
  ...
  const GRAPHICS_LABELS={eco:'Tiết kiệm',balanced:'Cân bằng',high:'Cao'};
  let graphicsMode=localStorage.getItem('doctor-rush-graphics-mode')||'auto';if(!['auto','eco','balanced','high'].includes(graphicsMode))graphicsMode='auto';
  function initialAutoQuality(){const mobile=isMobileLike(),mem=Number(navigator.deviceMemory||0),cores=Number(navigator.hardwareConcurrency||0);if(mobile&&((mem&&mem<=3)||(cores&&cores<=4)))return'eco';if(!mobile&&((mem&&mem<=4)||(cores&&cores<=4)))return'balanced';if((mem>=8&&cores>=8)||(!mobile&&cores>=8))return'high';return'balanced'}
  let audioCtx=null,lastBeepAt=0,assetsReady=false,quality=graphicsMode==='auto'?initialAutoQuality():graphicsMode,fps=60,fpsAccumMs=0,fpsFrames=0,goodFpsWindows=0,lowFpsWindows=0,rafPeak=0,autoEcoRenderFps=45,renderFps=60,renderFpsAccumMs=0,renderFrames=0,renderAccumulatorMs=0,lastFrameTs=performance.now(),simAccumulator=0,simSteps=0,pendingVersionUpdate=false,pendingControllerReload=false,autoPausedReason='',lastAutoToastAt=0;
  ...
  const LB_CONFIG=window.DOCTOR_RUSH_LEADERBOARD||{},LB_SEASON=Math.max(1,Number(LB_CONFIG.season||1)),LB_ENABLED=!!(LB_CONFIG.enabled&&LB_CONFIG.supabaseUrl&&LB_CONFIG.anonKey);
  function makePlayerId(){if(crypto.randomUUID)return crypto.randomUUID();return'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g,c=>{const r=Math.random()*16|0,v=c==='x'?r:(r&3|8);return v.toString(16)})}
  let playerId=localStorage.getItem('doctor-rush-player-id')||makePlayerId();localStorage.setItem('doctor-rush-player-id',playerId);
  let playerName=(localStorage.getItem('doctor-rush-player-name')||`Người chơi ${playerId.slice(0,4).toUpperCase()}`).slice(0,24),leaderDifficulty=difficulty,leaderPeriod='all',activeRunId=null;
  localStorage.setItem('doctor-rush-player-name',playerName);
  ...
  function qualityPick(eco,balanced,high){return quality==='eco'?eco:quality==='balanced'?balanced:high}
  function fxBlur(v){return qualityPick(0,v*.55,v)}
  ...
  function targetRenderFPS(){if(graphicsMode==='eco')return 30;if(graphicsMode==='auto'&&quality==='eco')return autoEcoRenderFps;return 60}
  function targetDPR(){const dpr=Math.max(1,devicePixelRatio||1);if(isMobileLike())return qualityPick(1,Math.min(dpr,1.25),Math.min(dpr,1.45));return qualityPick(Math.min(dpr,1.2),Math.min(dpr,1.5),Math.min(dpr,2))}
  function deviceProfileText(){const mem=navigator.deviceMemory?`${navigator.deviceMemory} GB RAM trình duyệt`:'RAM không công bố',cores=navigator.hardwareConcurrency?`${navigator.hardwareConcurrency} luồng CPU`:'CPU không công bố',hz=rafPeak?`${Math.round(rafPeak)} Hz đo được`:'đang đo tần số màn hình';return `${isMobileLike()?'Thiết bị cảm ứng/mobile':'PC/laptop'} • ${mem} • ${cores} • ${hz} • DPR ${DPR.toFixed(2)}`}
  ...
  function beginRun(){reset();state='playing';showPanel('');stage.classList.add('game-active');armGameHistory();totalStats.plays++;localStorage.setItem('doctor-rush-total-plays',totalStats.plays);leaderboardStartRun();beep(540,.08,'triangle',.05);vibrate(18)}
  ...
  function burst(x,y,n,type='spark'){const scale=qualityPick(.45,.72,1),limit=qualityPick(55,75,90),count=Math.max(3,Math.ceil(n*scale));for(let i=0;i<count;i++){if(particles.length>limit)particles.shift();particles.push({x,y,vx:(Math.random()-.5)*7,vy:(Math.random()-.75)*7,life:25+Math.random()*25,size:2+Math.random()*4,type})}}
  ...
  function bg(){
    ...
    skyline.forEach(b=>{
      ...
      if(b.brand){
        ...
        ctx.filter=quality==='eco'?'none':`blur(${quality==='balanced'?(b.brandMode==='rear'?4:3):(b.brandMode==='rear'?8:5)}px)`;
        ctx.beginPath();
  ...
  function syncGraphicsUI(){document.body.dataset.graphicsQuality=quality;const modeLabel=graphicsMode==='auto'?'AUTO':GRAPHICS_LABELS[graphicsMode];if($('graphicsBtn'))$('graphicsBtn').textContent=`${graphicsMode==='auto'?'🤖':'⚙️'} Đồ họa: ${modeLabel}`;document.querySelectorAll('[data-graphics-mode]').forEach(btn=>btn.classList.toggle('active',btn.dataset.graphicsMode===graphicsMode));if($('graphicsModeStatus'))$('graphicsModeStatus').textContent=modeLabel;if($('graphicsQualityStatus'))$('graphicsQualityStatus').textContent=GRAPHICS_LABELS[quality];if($('graphicsFpsStatus'))$('graphicsFpsStatus').textContent=`${fps.toFixed(0)} FPS`;if($('graphicsRenderStatus'))$('graphicsRenderStatus').textContent=`${targetRenderFPS()} FPS`;if($('graphicsDeviceStatus'))$('graphicsDeviceStatus').textContent=deviceProfileText()}
  function setQuality(next,reason='auto'){if(!['eco','balanced','high'].includes(next)||next===quality){syncGraphicsUI();return}quality=next;renderAccumulatorMs=0;resize();syncGraphicsUI();if(reason==='auto'&&state==='playing'&&performance.now()-lastAutoToastAt>9000){lastAutoToastAt=performance.now();showUpdateToast(next==='high'?'AUTO: đã tăng chất lượng hình ảnh':next==='balanced'?'AUTO: cân bằng lại đồ họa để giữ game mượt':'AUTO: đã giảm tải đồ họa để hạn chế giật lag',1500)}}
  function setGraphicsMode(mode){if(!['auto','eco','balanced','high'].includes(mode))return;graphicsMode=mode;localStorage.setItem('doctor-rush-graphics-mode',mode);goodFpsWindows=0;lowFpsWindows=0;autoEcoRenderFps=45;setQuality(mode==='auto'?initialAutoQuality():mode,'manual');syncGraphicsUI();showUpdateToast(mode==='auto'?'Đồ họa AUTO đã bật • game sẽ tự cân bằng theo FPS':`Đồ họa: ${GRAPHICS_LABELS[mode]}`,1500)}
  function updatePerformance(deltaMs){fpsAccumMs+=deltaMs;fpsFrames++;renderFpsAccumMs+=deltaMs;if(fpsAccumMs<2500)return;fps=fpsFrames*1000/fpsAccumMs;renderFps=renderFrames*1000/Math.max(1,renderFpsAccumMs);rafPeak=Math.max(rafPeak,fps);if(graphicsMode==='auto'&&['playing','menu','over'].includes(state)){const target=Math.max(30,Math.min(60,rafPeak||fps)),severe=fps<target*.67,weak=fps<target*.82,strong=fps>target*.95;if(quality==='high'){lowFpsWindows=severe?2:weak?lowFpsWindows+1:0;if(lowFpsWindows>=2){lowFpsWindows=0;goodFpsWindows=0;setQuality('balanced')}}else if(quality==='balanced'){lowFpsWindows=severe?2:weak?lowFpsWindows+1:0;if(lowFpsWindows>=2){lowFpsWindows=0;goodFpsWindows=0;setQuality('eco')}else if(strong){goodFpsWindows++;if(goodFpsWindows>=6){goodFpsWindows=0;setQuality('high')}}else goodFpsWindows=0}else{if(severe&&autoEcoRenderFps===45)autoEcoRenderFps=30;else if(strong&&autoEcoRenderFps===30){goodFpsWindows++;if(goodFpsWindows>=4){autoEcoRenderFps=45;goodFpsWindows=0}}else if(strong&&autoEcoRenderFps===45){goodFpsWindows++;if(goodFpsWindows>=7){goodFpsWindows=0;setQuality('balanced')}}else goodFpsWindows=0}}syncGraphicsUI();fpsAccumMs=0;fpsFrames=0;renderFpsAccumMs=0;renderFrames=0}
  function updateDebug(){if(!DEBUG)return;$('debugPanel').textContent=`${APP_VERSION}  build ${BUILD_ID}
RAF ${fps.toFixed(1)}  render ${renderFps.toFixed(1)}  cap ${targetRenderFPS()}
mode ${graphicsMode}  quality ${quality}  DPR ${DPR.toFixed(2)}
...
cache doctor-rush-v${BUILD_ID}`}
  function loop(ts){let deltaMs=ts-lastFrameTs;if(!Number.isFinite(deltaMs)||deltaMs<=0)deltaMs=FIXED_MS;lastFrameTs=ts;deltaMs=Math.min(100,deltaMs);updatePerformance(deltaMs);simAccumulator+=deltaMs;simSteps=0;while(simAccumulator>=FIXED_MS-0.0001&&simSteps<6){update(1);simAccumulator-=FIXED_MS;simSteps++}if(simSteps>=6)simAccumulator=Math.min(simAccumulator,FIXED_MS);renderAccumulatorMs+=deltaMs;const renderInterval=1000/targetRenderFPS();if(renderAccumulatorMs>=renderInterval-.35){render();renderFrames++;renderAccumulatorMs%=renderInterval}updateDebug();requestAnimationFrame(loop)}
  requestAnimationFrame(ts=>{lastFrameTs=ts;simAccumulator=0;requestAnimationFrame(loop)});
  async function supabaseRpc(name,payload){if(!LB_ENABLED)throw new Error('Leaderboard chưa cấu hình');const base=String(LB_CONFIG.supabaseUrl).replace(/\/$/,'');const r=await fetch(`${base}/rest/v1/rpc/${name}`,{method:'POST',headers:{'Content-Type':'application/json','apikey':LB_CONFIG.anonKey,'Authorization':`Bearer ${LB_CONFIG.anonKey}`},body:JSON.stringify(payload)});const text=await r.text();if(!r.ok)throw new Error(text||`HTTP ${r.status}`);if(!text)return null;try{return JSON.parse(text)}catch(e){return text}}
  function scalarRpcResult(value){if(Array.isArray(value))value=value[0];if(value&&typeof value==='object'){const keys=Object.keys(value);if(keys.length===1)return value[keys[0]]}return value}
  async function leaderboardStartRun(){activeRunId=null;if(!LB_ENABLED||!navigator.onLine)return;try{const out=await supabaseRpc('doctor_rush_start_run',{p_player_id:playerId,p_player_name:playerName,p_difficulty:difficulty,p_season:LB_SEASON,p_game_version:APP_VERSION});activeRunId=String(scalarRpcResult(out)||'')||null}catch(e){activeRunId=null}}
  async function leaderboardSubmitCurrentRun(result){const runId=activeRunId;activeRunId=null;if(!runId||!LB_ENABLED||!navigator.onLine)return;try{await supabaseRpc('doctor_rush_submit_run',{p_run_id:runId,p_player_id:playerId,p_score:result.score,p_items:result.items,p_gpp:result.gpp,p_duration_seconds:result.duration});showUpdateToast('🏆 Thành tích đã gửi lên bảng xếp hạng',1400);if(!$('leaderboardLayer').classList.contains('hidden'))refreshLeaderboard()}catch(e){console.warn('Leaderboard submit failed',e)}}
  function setLeaderboardStatus(text,online=false){$('leaderboardStatus').innerHTML=`<span class="online-dot ${online?'on':''}"></span>${text}`}
  function escapeHtml(v){return String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
  function localLeaderboardRows(){const s=getBest(leaderDifficulty);return s>0?[{rank:'—',display_name:playerName,score:s,items:0,local:true,is_me:true}]:[]}
  function renderLeaderboardRows(rows){const el=$('leaderboardList');if(!rows||!rows.length){el.innerHTML='<div class="leaderboard-status">Chưa có thành tích ở bộ lọc này.</div>';return}el.innerHTML=rows.map((r,i)=>{const rank=r.rank??i+1,medal=Number(rank)===1?'🥇':Number(rank)===2?'🥈':Number(rank)===3?'🥉':`#${rank}`,me=!!(r.is_me||r.local);return `<div class="leader-row${me?' me':''}"><div class="leader-rank">${medal}</div><div class="leader-name">${escapeHtml(r.display_name||'Người chơi')}${me?' • Bạn':''}</div><div class="leader-score">${Number(r.score||0).toLocaleString('vi-VN')}</div><div class="leader-meta">${r.local?'Kỷ lục máy':`${Number(r.items||0)} vật phẩm`}</div></div>`}).join('')}
  async function refreshLeaderboard(){$('myRankBox').hidden=true;if(!LB_ENABLED){setLeaderboardStatus('Chưa gắn Supabase • đang hiển thị kỷ lục lưu trên thiết bị.');renderLeaderboardRows(localLeaderboardRows());return}if(!navigator.onLine){setLeaderboardStatus('Đang offline • kỷ lục trên máy vẫn được giữ nguyên.');renderLeaderboardRows(localLeaderboardRows());return}setLeaderboardStatus('Đang tải bảng xếp hạng online…',true);$('leaderboardList').innerHTML='<div class="leaderboard-status">Đang tải…</div>';try{const rows=await supabaseRpc('doctor_rush_top',{p_viewer_id:playerId,p_difficulty:leaderDifficulty,p_period:leaderPeriod,p_season:LB_SEASON,p_limit:50});renderLeaderboardRows(Array.isArray(rows)?rows:[]);setLeaderboardStatus(`Online • ${difficultyConfigs[leaderDifficulty].label} • ${leaderPeriod==='today'?'Hôm nay':leaderPeriod==='week'?'Tuần này':'Mọi thời đại'}`,true);if(!Array.isArray(rows)||!rows.some(r=>r.is_me)){const mine=await supabaseRpc('doctor_rush_my_rank',{p_player_id:playerId,p_difficulty:leaderDifficulty,p_period:leaderPeriod,p_season:LB_SEASON});const r=Array.isArray(mine)?mine[0]:mine;if(r&&r.rank){$('myRankBox').textContent=`Hạng của bạn: #${r.rank} • ${Number(r.score||0).toLocaleString('vi-VN')} điểm`;$('myRankBox').hidden=false}}}catch(e){setLeaderboardStatus('Không tải được bảng online • đang dùng kỷ lục trên máy.');renderLeaderboardRows(localLeaderboardRows())}}
  function openLeaderboard(){leaderDifficulty=difficulty;$('playerNameInput').value=playerName;$('leaderboardSeason').textContent=`Mùa ${LB_SEASON} • mỗi người lấy thành tích tốt nhất.`;document.querySelectorAll('[data-leader-diff]').forEach(b=>b.classList.toggle('active',b.dataset.leaderDiff===leaderDifficulty));document.querySelectorAll('[data-leader-period]').forEach(b=>b.classList.toggle('active',b.dataset.leaderPeriod===leaderPeriod));$('leaderboardLayer').classList.remove('hidden');refreshLeaderboard()}
  function closeLeaderboard(){$('leaderboardLayer').classList.add('hidden')}
  function openGraphics(){syncGraphicsUI();$('graphicsLayer').classList.remove('hidden')}
  function closeGraphics(){$('graphicsLayer').classList.add('hidden')}
  ...
  document.querySelectorAll('.difficulty-option').forEach(btn=>btn.onclick=()=>selectDifficulty(btn.dataset.difficulty));selectDifficulty(difficulty);$('versionLabel').textContent=APP_VERSION;syncGraphicsUI();
  $('graphicsBtn').onclick=openGraphics;$('graphicsCloseBtn').onclick=closeGraphics;document.querySelectorAll('[data-graphics-mode]').forEach(btn=>btn.onclick=()=>setGraphicsMode(btn.dataset.graphicsMode));
  $('leaderboardBtn').onclick=openLeaderboard;$('gameOverLeaderboardBtn').onclick=openLeaderboard;$('leaderboardCloseBtn').onclick=closeLeaderboard;$('leaderboardRefreshBtn').onclick=refreshLeaderboard;
  document.querySelectorAll('[data-leader-diff]').forEach(btn=>btn.onclick=()=>{leaderDifficulty=btn.dataset.leaderDiff;document.querySelectorAll('[data-leader-diff]').forEach(x=>x.classList.toggle('active',x===btn));refreshLeaderboard()});
  document.querySelectorAll('[data-leader-period]').forEach(btn=>btn.onclick=()=>{leaderPeriod=btn.dataset.leaderPeriod;document.querySelectorAll('[data-leader-period]').forEach(x=>x.classList.toggle('active',x===btn));refreshLeaderboard()});
  $('savePlayerNameBtn').onclick=async()=>{const value=$('playerNameInput').value.trim().replace(/\s+/g,' ').slice(0,24);if(value.length<2){showUpdateToast('Tên hiển thị cần ít nhất 2 ký tự',1500);return}playerName=value;localStorage.setItem('doctor-rush-player-name',playerName);if(LB_ENABLED&&navigator.onLine)try{await supabaseRpc('doctor_rush_set_name',{p_player_id:playerId,p_player_name:playerName});showUpdateToast('✓ Đã lưu tên xếp hạng',1300)}catch(e){showUpdateToast('Đã lưu tên trên máy • sẽ đồng bộ ở ván online tiếp theo',1700)}refreshLeaderboard()};
